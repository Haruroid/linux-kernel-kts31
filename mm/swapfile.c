...
void si_swapinfo2(struct sysinfo2 *val)
{
	unsigned int type;
	unsigned long nr_to_be_unused = 0;
	...
	val->nr_to_be_unused = nr_to_be_unused;
	val->nr_swap_pages = atomic_long_read(&nr_swap_pages);
	val->total_swap_pages = total_swap_pages;
	val->freeswap = atomic_long_read(&nr_swap_pages) + nr_to_be_unused;
	val->totalswap = total_swap_pages + nr_to_be_unused;
	spin_unlock(&swap_lock);
}
EXPORT_SYMBOL(si_swapinfo2);
