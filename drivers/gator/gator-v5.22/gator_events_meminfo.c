...
static const char * const meminfo_names[] = {
	...
	"Linux_meminfo_swapfree",
	"Linux_meminfo_swaptotal",
	"Linux_meminfo_swapused",
	"Linux_meminfo_cmafree",
	"Linux_meminfo_cmatotal",
	"Linux_meminfo_memfree2",
	"Linux_meminfo_zram_used",
	"Linux_meminfo_zram_compr",
	"Linux_meminfo_zram_phyused",
};
...
static void do_read(void)
{
	...
	si_meminfo(&info);
	si_swapinfo(&info); // had spin_lock ??
	si_cmainfo(&cmainfo);
	...
	for (i = 0; i < MEMINFO_TOTAL; i++) {
		if (meminfo_enabled[i]) {
			...
			case MEMINFO_MEMUSED:
				...
				continue;
			case MEMINFO_SWAPFREE:
				value = info.freeswap * PAGE_SIZE;
				break;
			case MEMINFO_SWAPTOTAL:
				value = info.totalswap * PAGE_SIZE;
				break;
			case MEMINFO_SWAPUSED:
				value = ( info.totalswap - info.freeswap ) * PAGE_SIZE;
				break;
			case MEMINFO_CMAFREE:
				value = cmainfo.freecma * PAGE_SIZE;
				break;
			case MEMINFO_CMATOTAL:
				value = cmainfo.totalcma * PAGE_SIZE;
				break;
			case MEMINFO_MEMFREE2:
				value = info.freeram * PAGE_SIZE;
				break;
			case MEMINFO_ZRAMUSED:
				value = copy_zram_stats2.pages_stored * PAGE_SIZE;
				break;
			case MEMINFO_ZRAMCOMPR:
				value = copy_zram_stats2.compr_data_size;
				break;
			case MEMINFO_ZRAMPHYUSED:
				value = copy_zram_stats2.mem_used * PAGE_SIZE;
				break;
