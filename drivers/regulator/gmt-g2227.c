...
static int gmt_g2227_set_voltage_sel(struct regulator_dev *rdev, unsigned sel)
{
	...
	if(rdev->desc->vsel_reg==0x14) {
		while(1) {
			temp = val & 0xF0;
			if(temp!=0xA0) {
				printk(KERN_ERR "[PMIC] read error=0x%x\n", val);
	...
	if(temp!=val)
		printk(KERN_ERR "[PMIC] write error=0x%x, 0x%x, 0x%x\n", rdev->desc->vsel_reg, val, temp);
	else
		printk("[PMIC] write ok reg=0x%x, value=0x%x\n", rdev->desc->vsel_reg, val);

	return ret;	
//	return gmt_g2227_write(pmic_dev, rdev->desc->vsel_reg, val);
}
