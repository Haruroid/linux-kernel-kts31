...
static void release_pkthdr(struct sk_buff  *skb, int idx)
{
	...
	#if defined(CONFIG_RTD_1295_HWNAT)
	mapping = dma_map_single(&rtl819x_pdev->dev, skb->head, skb->truesize, DMA_FROM_DEVICE);
	...
	#if defined(CONFIG_RTD_1295_HWNAT)
	pReadyForHw->ph_mbuf->m_extbuf = pReadyForHw->ph_mbuf->m_data = (uint8 *) (mapping + skb_headroom(skb));
...
#if defined(CONFIG_RTD_1295_HWNAT)
__IRAM_FWD
static void increase_rx_idx_release_pkthdr(struct sk_buff  *skb, dma_addr_t addr, int idx)
#else //defined(CONFIG_RTD_1295_HWNAT)
__IRAM_FWD
...
#endif //defined(CONFIG_RTD_1295_HWNAT)
{
...
#if defined(CONFIG_RTL_ENHANCE_RELIABILITY) && (defined(CONFIG_RTL_8198C) || defined(CONFIG_RTL_8197F))
	mbufIndex = ((uintptr_t)(pReadyForHw->ph_mbuf) - rxMbufRing_base) /(sizeof(struct rtl_mBuf));
#else
	mbufIndex = ((uint32)(pReadyForHw->ph_mbuf) - (rxMbufRing[0] & ~(DESC_OWNED_BIT | DESC_WRAP))) /
					(sizeof(struct rtl_mBuf));
#endif
	...
	#if defined(CONFIG_RTD_1295_HWNAT)
	pReadyForHw->ph_mbuf->m_extbuf = pReadyForHw->ph_mbuf->m_data = (uint8 *) (addr + skb_headroom(skb));
...
int32 swNic_flushRxRingByPriority(int priority)
{
	...
	for(i = priority -1; i >= 0; i--)
	{
		...
		while((rxPkthdrRing[i][currRxPkthdrDescIndex[i]] & DESC_OWNED_BIT) == DESC_RISC_OWNED)
		{
			...
			#if defined(CONFIG_RTD_1295_HWNAT)
			increase_rx_idx_release_pkthdr((struct sk_buff *)skb, pPkthdr->dma_addr, i);
			#else //defined(CONFIG_RTD_1295_HWNAT)
	...
	#endif
	{
...
#if defined(CONFIG_RTD_1295_HWNAT)
		sync_rx_data_for_cpu((dma_addr_t)pPkthdr->ph_mbuf->m_data, pPkthdr->ph_len);
#else //defined(CONFIG_RTD_1295_HWNAT)
		...
		if(mbuf && mbuf->m_next)	/*we will process jumbo frame here*/
		{
			...
			while(buf_tmp!=NULL && mbuf!=NULL){
				#if defined(CONFIG_RTD_1295_HWNAT)
				sync_rx_data_for_cpu(mbuf->m_pkthdr->dma_addr, mbuf->m_len);
				pskb = (struct sk_buff *) mbuf->skb;
				memcpy(buf_tmp+offset, pskb->data, mbuf->m_len);
				#else //defined(CONFIG_RTD_1295_HWNAT)
			...
			#if defined(CONFIG_RTD_1295_HWNAT)
			// skb_tmp is cached memory for copy of jumbo frame.
			// reuse pPkthdr->ph_mbuf->skb for rx descriptor
			increase_rx_idx_release_pkthdr(pPkthdr->ph_mbuf->skb, pPkthdr->dma_addr, rxRingIdx);
			#else //defined(CONFIG_RTD_1295_HWNAT)
			increase_rx_idx_release_pkthdr(pPkthdr->ph_mbuf->skb, rxRingIdx);
			#endif //defined(CONFIG_RTD_1295_HWNAT)
			...
			while(m_next)
			{
				...
				#if defined(CONFIG_RTD_1295_HWNAT)
				m_next->m_extbuf = m_next->m_data = (uint8 *) (m_next->m_pkthdr->dma_addr + skb_headroom(pskb));
				#else //defined(CONFIG_RTD_1295_HWNAT)
		...
		if (buf)
		{
			...
			dma_unmap_single(&rtl819x_pdev->dev, pPkthdr->dma_addr,
				((struct sk_buff *)pPkthdr->ph_mbuf->skb)->truesize, DMA_FROM_DEVICE);
			skbp = (struct sk_buff *) skb;
			mapping = dma_map_single(&rtl819x_pdev->dev, skbp->head, skbp->truesize, DMA_FROM_DEVICE);
			if (unlikely(dma_mapping_error(&rtl819x_pdev->dev, mapping))) {
				DBG("Failed to map RX DMA memory!");
...
static inline int get_protocol_type(struct sk_buff *skb, struct rtl_pktHdr *pPkthdr)
{
	...
	if(*(int16 *)(skb->data+12)==(int16)htons(ETH_P_IP)) {
		...
		if ((*(int16 *)(skb->data+12+4)==(int16)htons(ETH_P_PPP_SES)) ||
...
__IRAM_FWD  inline int32 _swNic_send(void *skb, void * output, uint32 len,rtl_nicTx_info *nicTx)
{
	...
	#if defined(CONFIG_8198_PORT5_GMII) || defined(CONFIG_8198_PORT5_RGMII) || defined(CONFIG_8198C_8211FS)
...
#ifdef CONFIG_RTK_VLAN_WAN_TAG_SUPPORT
#if defined(CONFIG_RTD_1295_HWNAT)
if (*((unsigned short *)((unsigned char*)skb->data + ETH_ALEN*2)) != __constant_htons(ETH_P_8021Q))
...
if (*((unsigned short *)((unsigned char*)output+ETH_ALEN*2)) != __constant_htons(ETH_P_8021Q))
...
#endif //defined(CONFIG_RTD_1295_HWNAT)
	{
			...
			if(pPkthdr->ph_portlist == RTL_WANPORT_MASK)
				pPkthdr->ph_txCVlanTagAutoAdd = pPkthdr->ph_vlanId!=RTL_WANVLANID ? RTL_WANPORT_MASK : 0;
			else
	...
	if(pPkthdr->ph_portlist == RTL_WANPORT_MASK && (pPkthdr->ph_txCVlanTagAutoAdd & RTL_WANPORT_MASK))
	{
	...
	for (i = 0; i < skb_shinfo(skb)->nr_frags; i++)
	{
		...
		if (i == (skb_shinfo(skb)->nr_frags-1)) { // last one
...
#if defined(CONFIG_RTD_1295_HWNAT)
	sync_tx_data_for_device((dma_addr_t) output, ((struct sk_buff *)skb)->len);
#else //defined(CONFIG_RTD_1295_HWNAT)
#ifdef _PKTHDR_CACHEABLE
	#if defined(CONFIG_RTL_8198C) || defined(CONFIG_RTL_8197F)
...
int32 swNic_txDone(int idx)
{
	...
	//free_num = 0;
	{
			...
			& DESC_OWNED_BIT) == DESC_RISC_OWNED )
		{
			...
			{
				#if defined(CONFIG_RTD_1295_HWNAT)
				tx_skb = (struct sk_buff *) pPkthdr->ph_mbuf->skb;
				#endif //defined(CONFIG_RTD_1295_HWNAT)
				...
				#else
				#if defined(CONFIG_RTD_1295_HWNAT)
				/* unmap dma range of  TX skb->data here */
				dma_unmap_single(&rtl819x_pdev->dev, pPkthdr->dma_addr, tx_skb->len, DMA_TO_DEVICE);
				#endif //defined(CONFIG_RTD_1295_HWNAT)
...
void swNic_freeRxBuf(void)
{
	...
	if (rxMbufRing) {
		...
		for (i=0;i<rxMbufRingCnt;i++)
		{
			...
			{
				#if defined(CONFIG_RTD_1295_HWNAT)
				dma_unmap_single(&rtl819x_pdev->dev, pMbuf->m_pkthdr->dma_addr, ((struct sk_buff *) pMbuf->skb)->truesize, DMA_FROM_DEVICE);
				 ...
				 uint32 clusterSize)
{
	...
	if (rxMbufRing == NULL)
	{
		...
		for (i = 0; i < RTL865X_SWNIC_RXRING_HW_PKTDESC; i++) {
			...
			#if defined(CONFIG_RTD_1295_HWNAT)
			rxPkthdrRing[i] = (uintptr_t*) dma_alloc_coherent(&rtl819x_pdev->dev, rxPkthdrRingCnt[i] * sizeof(uintptr_t*),
		...
		for (i = 0; i < RTL865X_SWNIC_TXRING_HW_PKTDESC; i++) {
			...
			#if defined(CONFIG_RTD_1295_HWNAT)
			txPkthdrRing[i] = (uintptr_t*) dma_alloc_coherent(&rtl819x_pdev->dev, txPkthdrRingCnt[i] * sizeof(uintptr_t*),
		...
		#if defined(CONFIG_RTD_1295_HWNAT)
		rxMbufRing = (uintptr_t*) dma_alloc_coherent(&rtl819x_pdev->dev, (rxMbufRingCnt+RESERVERD_MBUF_RING_NUM) * sizeof(uintptr_t*),
...
#else
		#if defined(CONFIG_RTD_1295_HWNAT)
		pPkthdrList_start = (struct rtl_pktHdr *) dma_alloc_coherent(&rtl819x_pdev->dev,
		...
		/* Allocate mbufs */
		pMbufList_start = (struct rtl_mBuf *) dma_alloc_coherent(&rtl819x_pdev->dev,
	...
	for (i = 0; i < RTL865X_SWNIC_TXRING_HW_PKTDESC; i++)
	{
		for (j = 0; j < txPkthdrRingCnt[i]; j++)
		{
			...
			txPkthdrRing[i][j] = (uintptr_t) pPkthdr | DESC_RISC_OWNED;
			#ifdef CONFIG_RTL8196C_REVISION_B
			if (rtl_chip_version == RTL8196C_REVISION_A)
	...
	DBG("Initialize Rx packet header descriptors\n");
	...
	for (i = 0; i < RTL865X_SWNIC_RXRING_HW_PKTDESC; i++)
	{
		...
		for (j = 0; j < rxPkthdrRingCnt[i]; j++)
		{
			...
			if (alloc_rx_buf(&pPkthdr->ph_mbuf->skb, size_of_cluster) > 0)
			{
				...
				pMbuf->m_data = pMbuf->m_extbuf = (uint8 *) (mapping + skb_headroom(skbp));
			...
			{
				/* Setup descriptors */
				rxPkthdrRing[i][j] = (uintptr_t) pPkthdr | DESC_SWCORE_OWNED;
				rxMbufRing[k++] = (uintptr_t) pMbuf | DESC_SWCORE_OWNED;
			}
#endif
		}
