...
#if defined(CONFIG_RTD_1295_HWNAT)
__DRAM_FWD static uintptr_t New_txDescRing_base[RTL865X_SWNIC_TXRING_HW_PKTDESC];
__DRAM_FWD static uintptr_t New_rxDescRing_base[RTL865X_SWNIC_RXRING_HW_PKTDESC];
...
void New_dumpTxRing(struct seq_file *s)
{
	...
	for (i = 0; i < NEW_NIC_MAX_TX_DESC_RING; i++)
	{
		...
		{
			#if defined(CONFIG_RTD_1295_HWNAT)
			seq_printf(s,"tx_desc[%d][%d]: 0x%lx,",i,j,(uintptr_t)(&New_txDescRing[i][j]) );
			#else //defined(CONFIG_RTD_1295_HWNAT)
	...
	for (i = 0; i < NEW_NIC_MAX_TX_DESC_RING; i++)
	{
		...
		for (j = 0; j < New_txDescRingCnt[i]; j++)
		{
			...
			#if defined(CONFIG_RTD_1295_HWNAT)
			if (skb == NULL)
				seq_printf(s,"[%d][%d]: tx_skb: %lx, skb: NULL\n",i,j,
					(uintptr_t)(&tx_skb[i][j]) );
			else
				seq_printf(s,"[%d][%d]: tx_skb: %lx, skb: %lx, skb->data: %lx,"
					" dma_addr: %lx, dma_len %d\n",i,j,
					(uintptr_t)(&tx_skb[i][j]), (uintptr_t)tx_skb[i][j].skb,
					(uintptr_t)skb->data, (uintptr_t)(tx_skb[i][j].dma_addr),
					tx_skb[i][j].dma_len);
			#else //defined(CONFIG_RTD_1295_HWNAT)
			if (skb	== NULL)
...
void New_dumpRxRing(struct seq_file *s)
{
	...
	for (i = 0; i < NEW_NIC_MAX_RX_DESC_RING; i++)
	{
		...
		{
			#if defined(CONFIG_RTD_1295_HWNAT)
			seq_printf(s,"rx_desc[%d][%d]: 0x%lx,",i,j,(uintptr_t)(&New_rxDescRing[i][j]) );
			#else //defined(CONFIG_RTD_1295_HWNAT)
	...
	for (i = 0; i < NEW_NIC_MAX_RX_DESC_RING; i++)
	{
		...
		for (j = 0; j < New_rxDescRingCnt[i]; j++)
		{
			...
			#if defined(CONFIG_RTD_1295_HWNAT)
			if (skb == NULL)
				seq_printf(s,"[%d][%d]: rx_skb: %lx, skb: NULL\n",i,j,
					(uintptr_t)(&rx_skb[i][j]) );
			else
				seq_printf(s,"[%d][%d]: rx_skb: %lx, skb: %lx, skb->data: %lx,"
					" dma_addr: %lx, dma_len %d\n",i,j,
					(uintptr_t)(&rx_skb[i][j]), (uintptr_t)rx_skb[i][j].skb,
					(uintptr_t)skb->data, (uintptr_t)(rx_skb[i][j].dma_addr),
					rx_skb[i][j].dma_len);
			#else //defined(CONFIG_RTD_1295_HWNAT)
			if (skb	== NULL)
		 ...
		 uint32 clusterSize)
{
	...
	for (i = 0; i < NEW_NIC_MAX_RX_DESC_RING; i++) {
		...
		#if defined(CONFIG_RTD_1295_HWNAT)
		New_rxDescRing[i] = (struct dma_rx_desc *) dma_alloc_coherent(&rtl819x_pdev->dev, New_rxDescRingCnt[i] * sizeof(struct dma_rx_desc),
	...
	for (i = 0; i < NEW_NIC_MAX_TX_DESC_RING; i++) {
		...
		#if defined(CONFIG_RTD_1295_HWNAT)
		New_txDescRing[i] = (struct dma_tx_desc *) dma_alloc_coherent(&rtl819x_pdev->dev, New_txDescRingCnt[i] * sizeof(struct dma_tx_desc),
	...
	for (i = 0; i < NEW_NIC_MAX_RX_DESC_RING; i++)
	{
		for (j = 0; j < New_rxDescRingCnt[i]; j++)
		{
			...
			if (alloc_rx_buf((void **)&skb, clusterSize) > 0)
			{
				...
				mapping = dma_map_single(&rtl819x_pdev->dev, skbp->data,
...
get_next:
		...
		DBG("rx desc = &New_rxDescRing[ring_idx %d][rx_idx %d], "
			...
			ring_idx, rx_idx, desc->mdata, desc->opts1.dw, desc->opts2.dw, desc->opts3.dw,
			desc->opts4.dw, desc->opts5.dw);
		...
		if (buf) {
			#if defined(CONFIG_RTD_1295_HWNAT)
			skbp = (struct sk_buff *) skb;
			mapping = dma_map_single(&rtl819x_pdev->dev, skbp->data, size_of_cluster, DMA_FROM_DEVICE);
			if (unlikely(dma_mapping_error(&rtl819x_pdev->dev, mapping))) {
				DBG("Failed to map RX DMA memory!");
				dev_kfree_skb_any(skbp);
				#if !defined(CONFIG_RTD_1295_HWNAT)
			 ...
			 */
			#if defined(CONFIG_RTD_1295_HWNAT)
			DBG("rx_skb[ring_idx %d][rx_idx %d] = 0x%lx, data = 0x%lx, len = %d", ring_idx, rx_idx, (uintptr_t) r_skb, (uintptr_t) r_skb->data, r_skb->len);
			...
			dma_unmap_single(&rtl819x_pdev->dev, rx_skb[ring_idx][rx_idx].dma_addr,
					rx_skb[ring_idx][rx_idx].dma_len, DMA_FROM_DEVICE);
			#else //defined(CONFIG_RTD_1295_HWNAT)
...
int32 _New_swNic_send_tso_sg(struct sk_buff *skb, void * output, uint32 len,rtl_nicTx_info *nicTx)
{
	...
	for (cur_frag = 0; cur_frag < skb_shinfo(skb)->nr_frags; cur_frag++)
	{
		...
		#if defined(CONFIG_RTD_1295_HWNAT)
		mapping = dma_map_single(&rtl819x_pdev->dev, addr, frag->size, DMA_TO_DEVICE);
		if (unlikely(dma_mapping_error(&rtl819x_pdev->dev, mapping))) {
			if (net_ratelimit())
...
int32 New_swNic_txDone(int idx)
{
	...
	while (New_txPktDoneDescIndex[idx] != hw_cdp_idx) {
		...
		if (dma_addr > 0)
		{
			dma_unmap_single(&rtl819x_pdev->dev, dma_addr, dma_len, DMA_TO_DEVICE);
	...
	for (i = 0; i < NEW_NIC_MAX_RX_DESC_RING; i++)
	{
		if (rx_skb[i]){
			for (j = 0; j < New_rxDescRingCnt[i]; j++)
			{
				...
				{
					#if defined(CONFIG_RTD_1295_HWNAT)
					dma_unmap_single(&rtl819x_pdev->dev, rx_skb[i][j].dma_addr, rx_skb[i][j].dma_len, DMA_FROM_DEVICE);
