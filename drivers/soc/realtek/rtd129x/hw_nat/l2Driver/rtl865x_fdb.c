...
int32 rtl865x_LinkChange_Process(void)
{
	...
	for ( i = 0; i < RTL8651_MAC_NUMBER; i++ )
	{
		...
		if ( ( status & PortStatusLinkUp ) == FALSE )
		{
			/* Link is down. */
			#if defined(CONFIG_RTD_1295_HWNAT)
			DBG("MAC#%d is link-down, status 0x%x", i, status);
			#endif //defined(CONFIG_RTD_1295_HWNAT)
		...
		else
		{
			/* Link is up. */
			#if defined(CONFIG_RTD_1295_HWNAT)
			if (status & PortStatusLinkSpeed1000M)
				speed = "1000M";
			else if (status & PortStatusLinkSpeed100M)
				speed = "100M";
			else if (status & PortStatusLinkSpeed10M)
				speed = "10M";
			else
				speed = "unknown";
			DBG("MAC#%d is link-up: status 0x%x, speed %s, %s duplex", i, status, speed, (status & PortStatusDuplex)? "full" : "half");
			#endif //defined(CONFIG_RTD_1295_HWNAT)
