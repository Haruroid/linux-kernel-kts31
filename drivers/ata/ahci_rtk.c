...
static void config_sata_phy(unsigned int port)
{
	...
	if(ahci_dev->spread_spectrum==0) {
		...
		writel_delay(0x39910811, base + 0xF60);
		writel_delay(0x39914811, base + 0xF60);
		writel_delay(0x39918811 , base + 0xF60);
...
/*	if(ahci_dev->port[port]->phy_status==0) {
	...
	if(ahci_dev->tx_driving==2) {  //TX_2: Gen3_MFTP_max_683mV
		...
		writel_delay(0x587aa111, base + 0xF60);
	...
	else if(ahci_dev->tx_driving==3) {  //TX_3: Gen3_MFTP_max_573mV
                printk("[SATA] set tx-driving to L (level 3)\n");
                writel_delay(0x94a62011, base + 0xF60);
                writel_delay(0x94a66011, base + 0xF60);
                writel_delay(0x94a6a011, base + 0xF60);
                writel_delay(0x486a2111, base + 0xF60);
                writel_delay(0x486a6111, base + 0xF60);
                writel_delay(0x486aa111, base + 0xF60);
        }
	else if(ahci_dev->tx_driving==4) {  //TX_4: Gen3_MFTP_max_500mV
                printk("[SATA] set tx-driving to L (level 4)\n");
                writel_delay(0x94a52011, base + 0xF60);
                writel_delay(0x94a56011, base + 0xF60);
                writel_delay(0x94a5a011, base + 0xF60);
                writel_delay(0x385a2111, base + 0xF60);
                writel_delay(0x385a6111, base + 0xF60);
                writel_delay(0x385aa111, base + 0xF60);
        }
	else if(ahci_dev->tx_driving==5) {  //TX_5: Gen3_MFTP_max_416mV
                ...
                writel_delay(0x384a2111, base + 0xF60);
                writel_delay(0x384a6111, base + 0xF60);
                writel_delay(0x384aa111, base + 0xF60);
        }
	else if(ahci_dev->tx_driving==6) {  //TX_6: Gen3_MFTP_max_303mV
                ...
                writel_delay(0x283a2111, base + 0xF60);
                writel_delay(0x283a6111, base + 0xF60);
                writel_delay(0x283aa111, base + 0xF60);
        }
// RX power saving off
		...
		if(host != NULL){
			if(host->ports[0]->scsi_host->shost_state == SHOST_RUNNING ){
				if(host->ports[0]->link.device[0].sdev != NULL){
					...
					RTK_SATA_DEV_FLAG = true;
				}
			}else if(host->ports[0]->scsi_host->shost_state == SHOST_RECOVERY){
				printk("[SATA] ata_scsi_user_scan start\n");
				ata_scsi_user_scan(host->ports[0]->scsi_host, 0, 0, 0);
				RTK_SATA_DEV_FLAG = false;
			}
		}
		set_current_state(TASK_INTERRUPTIBLE);
	}
