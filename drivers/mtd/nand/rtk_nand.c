...
//#include <asm/mach-rtk_dmp/padlock.h>

//#include <asm/mach-rtk_dmp/platform.h>
//#include <asm/mach-rtk_dmp/venus.h>
...
void __iomem	    *clk_base = 0;
void __iomem	    *sb2_base = 0;
...
static int rtk_read_ecc_page(struct mtd_info *mtd, u16 chipnr, unsigned int page,
			u_char *data, u_char *oob_buf, u16 cp_mode);
...
static int rtk_nand_read_ecc_status(u_char id[6])
{
	...
	REG_WRITE_U32(REG_ND_CMD,NF_ND_CMD_cmd(0x7A));
...
//	printk("AT[%s]:page %d, ppb %d, mtd->erasesize %u, mtd->writesize %u\n",__FUNCTION__,page,ppb,mtd->erasesize,mtd->writesize);
	...
	REG_WRITE_U32( REG_MULTI_CHNL_MODE+map_base,NF_MULTI_CHNL_MODE_no_wait_busy(1)|NF_MULTI_CHNL_MODE_edo(0));
...
//----------------------------------------------------------------------------
static int rtk_read_ecc_page (struct mtd_info *mtd, u16 chipnr, unsigned int page, u_char *data_buf, u_char *oob_buf, u16 cp_mode)
{
//static unsigned int eccReadCnt = 0;
	...
	while (1) 
	{
		...
		//Set ECC
		REG_WRITE_U32( REG_MULTI_CHNL_MODE+map_base,NF_MULTI_CHNL_MODE_edo(0));//add by alexchang0205-2010
		REG_WRITE_U32( REG_ECC_STOP+map_base,NF_ECC_STOP_ecc_n_stop(0x01));
...
static int rtk_nand_profile (void)
{
	...
	if(g_sw_WP_level == -1)
	{
		printk("Disable software write protect!\n");
	}
	else
		printk("Enable software write protect!\n");
	...
	printk("[%s] QQQ mtd_device_parse_register rc = 0x%x\n", __FUNCTION__, rc);
	//if (pnum <= 0) {
...
static int rtk_nand_probe(struct platform_device *pdev)
{
	...
	struct device_node *rtk119x_nand_node;
	rtk119x_nand_node = pdev->dev.of_node;
...
static struct of_device_id rtk119x_nand_ids[] = {
	{ .compatible = "Realtek,nand" },
	{ /* Sentinel */ },
};
...
static struct platform_driver rtk_nand_driver = {
	...
	.driver = {
		...
		.owner = THIS_MODULE,
		.of_match_table = rtk119x_nand_ids,
	},
};
...
static int __init rtk_nand_init (void)
{
	...
	u32 reg_base =0;
	u32 reg_base_len =0;
	dt_nand = of_find_matching_node(NULL, rtk119x_nand_ids);
	if (!dt_nand)
		printk(KERN_INFO "-->%s==>no dt_nand node\n", __func__);
	else{
		if (of_property_read_u32(dt_nand, "nand-enable", &nand_enable)){
			//printk("No nand-enable property for %s", dt_nand->name);
		}
		else
			printk(KERN_INFO "-->%s==>nand-enable property: %d\n", __func__,nand_enable);

		if (of_property_read_u32_index(dt_nand, "reg", 0, &nand_base)){
			printk("No nand_base property\n");
		}
		else
			printk(KERN_INFO "-->%s==>nand_base property: %x\n", __func__,nand_base);

		if (of_property_read_u32_index(dt_nand, "reg", 1, &nand_base_len)){
			printk("No nand_base_len property\n");
		}
		else
			printk(KERN_INFO "-->%s==>nand_base_len property: %x\n", __func__,nand_base_len);

		if (of_property_read_u32_index(dt_nand, "reg", 2, &reg_base)){
			printk("No reg_base property\n");
		}
		else
			printk(KERN_INFO "-->%s==>reg_base property: %x\n", __func__,reg_base);

		if (of_property_read_u32_index(dt_nand, "reg", 3, &reg_base_len)){
			printk("No reg_base_len property\n");
		}
		else
			printk(KERN_INFO "-->%s==>reg_base_len property: %x\n", __func__,reg_base_len);

	}
	
	unsigned long long pin_WP_info;

	//mantis : 42978 , swc/nwc switch error sw work around

map_base =  ioremap(nand_base, nand_base_len);
    printk(KERN_INFO "map_base:0x%08x\n",map_base);


clk_base =  ioremap(reg_base, reg_base_len);
    printk(KERN_INFO "clk_base:0x%08x\n",clk_base);

    sb2_base =  ioremap(0x9801a000, 0x100);
    printk(KERN_INFO "sb2_base:0x%08x\n",sb2_base);
//u32 read_value32= readl(map_base+REG_NF_LOW_PWR);
...
//printk(KERN_INFO "read_value32:0x%08x\n",read_value32);
	...
	#if RTK_NAND_INTERRUPT
	if(is_darwin_cpu()||is_macarthur_cpu()||is_nike_cpu()||is_macarthur2_cpu())
	{
