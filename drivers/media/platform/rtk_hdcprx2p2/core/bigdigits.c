...
/*******************************/
#ifndef NO_ALLOCS
DIGIT_T *mpAlloc(uint32_t ndigits)
{
	DIGIT_T *ptr;
...
/************************/
DIGIT_T mpAdd(DIGIT_T w[], const DIGIT_T u[], const DIGIT_T v[], uint32_t ndigits)
{
	/*      Calculates w = u + v
...
DIGIT_T mpSubtract(DIGIT_T w[], const DIGIT_T u[], const DIGIT_T v[], uint32_t ndigits)
{
...
int mpMultiply(DIGIT_T w[], const DIGIT_T u[], const DIGIT_T v[], uint32_t ndigits)
{
...
/* mpDivide */
static DIGIT_T mpMultSub(DIGIT_T wn, DIGIT_T w[], const DIGIT_T v[], DIGIT_T q, uint32_t n)
{
	/*      Compute w = w - qv
	where w = (WnW[n-1]...W[0])
	return modified Wn.
...
int mpDivide(DIGIT_T q[], DIGIT_T r[], const DIGIT_T u[], uint32_t udigits, DIGIT_T v[], uint32_t vdigits)
{
...
int mpSquare(DIGIT_T w[], const DIGIT_T x[], uint32_t ndigits)
/* New in Version 2.0 */
{
...
int mpEqual(const DIGIT_T a[], const DIGIT_T b[], uint32_t ndigits)
{
...
int mpCompare(const DIGIT_T a[], const DIGIT_T b[], uint32_t ndigits)
{
	/*      Returns sign of (a - b)*/
	if (ndigits == 0)
		return 0;
...
int mpIsZero(const DIGIT_T a[], uint32_t ndigits)
{
...
uint32_t mpSizeof(const DIGIT_T a[], uint32_t ndigits)
{
...
uint32_t mpBitLength(const DIGIT_T d[], uint32_t ndigits)
...
void mpSetEqual(DIGIT_T a[], const DIGIT_T b[], uint32_t ndigits)
{
	/* Sets a = b */
	uint32_t i;
	for (i = 0; i < ndigits; i++) {
...
volatile DIGIT_T mpSetZero(volatile DIGIT_T a[], uint32_t ndigits)
{
	/* Sets a = 0 */
...
void mpSetDigit(DIGIT_T a[], DIGIT_T d, uint32_t ndigits)
{
...
/* BIT-WISE FUNCTIONS */
/**********************/
DIGIT_T mpShiftLeft(DIGIT_T a[], const DIGIT_T *b, uint32_t shift, uint32_t ndigits)
{
	/* Computes a = b << shift */

	/* [v2.1] Modified to cope with shift > BITS_PERDIGIT */
	uint32_t i, y, nw, bits;
...
DIGIT_T mpShiftRight(DIGIT_T a[], const DIGIT_T b[], uint32_t shift, uint32_t ndigits)
{
	/* Computes a = b >> shift */
...
int mpSetBit(DIGIT_T a[], uint32_t ndigits, uint32_t ibit, int value)
	/* Set bit n (0..nbits-1) with value 1 or 0 */
{
	uint32_t idigit, bit_to_set;
	DIGIT_T mask;
...
int mpGetBit(DIGIT_T a[], uint32_t ndigits, uint32_t ibit)
	/* Returns value 1 or 0 of bit n (0..nbits-1); or -1 if out of range */
{
	uint32_t idigit, bit_to_get;
	DIGIT_T mask;
...
void mpModPowerOf2(DIGIT_T a[], uint32_t ndigits, uint32_t L)
...
void mpXorBits(DIGIT_T a[], const DIGIT_T b[], const DIGIT_T c[], uint32_t ndigits)
...
void mpOrBits(DIGIT_T a[], const DIGIT_T b[], const DIGIT_T c[], uint32_t ndigits)
...
void mpAndBits(DIGIT_T a[], const DIGIT_T b[], const DIGIT_T c[], uint32_t ndigits)
...
void mpNotBits(DIGIT_T a[], const DIGIT_T b[], uint32_t ndigits)
...
/*****************************************/
DIGIT_T mpShortAdd(DIGIT_T w[], const DIGIT_T u[], DIGIT_T v, uint32_t ndigits)
{
	/*      Calculates w = u + v
...
DIGIT_T mpShortSub(DIGIT_T w[], const DIGIT_T u[], DIGIT_T v, uint32_t ndigits)
{
	/*      Calculates w = u - v
	       where w, u are multiprecision integers of ndigits each
	       and v is a single precision digit.
...
DIGIT_T mpShortMult(DIGIT_T w[], const DIGIT_T u[], DIGIT_T v, uint32_t ndigits)
{
	/*      Computes product w = u * v
	       Returns overflow k
	       where w, u are multiprecision integers of ndigits each
...
DIGIT_T mpShortDiv(DIGIT_T q[], const DIGIT_T u[], DIGIT_T v, uint32_t ndigits)
{
...
DIGIT_T mpShortMod(const DIGIT_T a[], DIGIT_T d, uint32_t ndigits)
{
...
int mpShortCmp(const DIGIT_T a[], DIGIT_T b, uint32_t ndigits)
{
...
/*********************************/
/* Added [v2.2] */
int mpIsNegative(const DIGIT_T x[], uint32_t ndigits)
	/* Returns TRUE (1) if x < 0, else FALSE (0) */
{
	return ((x[ndigits - 1] & HIBITMASK) != 0);
}

int mpChs(DIGIT_T x[], const DIGIT_T y[], uint32_t ndigits)
	/* Sets x = -y */
{
...
int mpAbs(DIGIT_T x[], const DIGIT_T y[], uint32_t ndigits)
	/* Sets x = |y| */
{
	int isneg = mpIsNegative(y, ndigits);
	if (isneg)
...
void mpPrint(const DIGIT_T *p, uint32_t len)
{
	while (len--) {
...
void mpPrintNL(const DIGIT_T *p, uint32_t len)
{
	uint32_t i = 0;
	while (len--) {
		if ((i % 8) == 0 && i)
...
void mpPrintTrim(const DIGIT_T *p, uint32_t len)
{
	/* Trim leading zeroes */
	while (len--) {
		if (p[len] != 0)
...
void mpPrintTrimNL(const DIGIT_T *p, uint32_t len)
{
	 /* Trim leading zeroes */
	while (len--) {
		if (p[len] != 0)
...
/************************/
uint32_t mpConvFromOctets(DIGIT_T a[], uint32_t ndigits, const unsigned char *c, uint32_t nbytes)
...
uint32_t mpConvToOctets(const DIGIT_T a[], uint32_t ndigits, unsigned char *c, uint32_t nbytes)
...
*/
{
	...
	for (; j >= 0; j--)
		c[j] = 0;
	return (uint32_t) noctets;
}

/***************************/
/* NUMBER THEORY FUNCTIONS */
/***************************/
int mpModulo(DIGIT_T r[], const DIGIT_T u[], uint32_t udigits, DIGIT_T v[], uint32_t vdigits)
{
	     ...
	     */
	uint32_t nn = max(udigits, vdigits);
...
int mpModMult(DIGIT_T a[], const DIGIT_T x[], const DIGIT_T y[], DIGIT_T m[], uint32_t ndigits)
...
int mpModInv(DIGIT_T inv[], const DIGIT_T u[], const DIGIT_T v[], uint32_t ndigits)
{
...
int mpGcd(DIGIT_T g[], const DIGIT_T x[], const DIGIT_T y[], uint32_t ndigits)
{
...
int mpSqrt(DIGIT_T s[], const DIGIT_T x[], uint32_t ndigits)
...
int mpJacobi(const DIGIT_T a[], const DIGIT_T n[], uint32_t ndigits)
...
#ifdef NO_ALLOCS
static int mpModExp_1(DIGIT_T y[], const DIGIT_T x[], const DIGIT_T n[], DIGIT_T d[], uint32_t ndigits);

#else
static int mpModExp_windowed(DIGIT_T y[], const DIGIT_T x[], const DIGIT_T n[], DIGIT_T d[], uint32_t ndigits);

#endif
int mpModExp(DIGIT_T y[], const DIGIT_T x[], const DIGIT_T n[], DIGIT_T d[], uint32_t ndigits)
...
static int mpModExp_1(DIGIT_T yout[], const DIGIT_T x[], const DIGIT_T e[], DIGIT_T m[], uint32_t ndigits)
...
*/
static uint32_t WindowLenTable[] = {
/* k=1   2   3   4    5     6     7     8 */
...
*/
static int mpModExp_windowed(DIGIT_T yout[], const DIGIT_T g[], const DIGIT_T e[], DIGIT_T m[], uint32_t ndigits)
/* Computes y = g^e mod m using sliding-window exponentiation */
{
	uint32_t nbits;		/* Number of significant bits in e */
	uint32_t winlen;		/* Window size */
	DIGIT_T mask;		/* Bit mask used for main loop */
	uint32_t nwd;		/* Digit counter for main loop */
	DIGIT_T lkamask;	/* Bit mask for lookahead */
	uint32_t lkanwd;		/* Digit counter for lookahead */
	DIGIT_T lkabuf;	/* One-digit lookahead buffer */
	uint32_t lkalen;		/* Actual size of window for current lookahead buffer */
	int in_window;		/* Flag for in-window state */
	DIGIT_T *gtable[(1 << (WINLENTBLMAX - 1))];	/* Table of ptrs to g1, g3, g5,... */
	uint32_t ngt;		/* No of elements in gtable */
	uint32_t idxmult;	/* Index (in gtable) of next multiplier to use: 0=g1, 1=g3, 2=g5,... */
	DIGIT_T *g2;		/* g2 = g^2 */
	DIGIT_T *temp1, *temp2;	/* Temp big digits, needed for MULT and SQUARE macros */
	DIGIT_T *a;		/* A */
	int aisone;		/* Flag that A == 1 */
	uint32_t nn;		/* 2 * ndigits */
	uint32_t i;		/* Temp counter */
