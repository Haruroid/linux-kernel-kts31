...
#define MAX_HALF_DIGIT 0x0000ffff	/* NB 'L' */
...
int mpModExp(DIGIT_T y[], const DIGIT_T x[], const DIGIT_T e[], DIGIT_T m[], uint32_t ndigits);
...
/* BITWISE OPERATIONS */
/**********************/
uint32_t mpBitLength(const DIGIT_T a[], uint32_t ndigits);
	    /* Returns number of significant bits in a */
DIGIT_T mpShiftLeft(DIGIT_T a[], const DIGIT_T b[], uint32_t x, uint32_t ndigits);
	    /* Computes a = b << x */
DIGIT_T mpShiftRight(DIGIT_T a[], const DIGIT_T b[], uint32_t x, uint32_t ndigits);
	    /* Computes a = b >> x */
...
void mpModPowerOf2(DIGIT_T a[], uint32_t ndigits, uint32_t L);
 ...
 int mpSetBit(DIGIT_T a[], uint32_t ndigits, uint32_t n, int value);
	    /* Set bit n (0..nbits-1) with value 1 or 0 */
int mpGetBit(DIGIT_T a[], uint32_t ndigits, uint32_t n);
...
int mpIsPrime(DIGIT_T w[], uint32_t ndigits, uint32_t t);
	    /* Returns true if w > 2 is a probable prime
	       t tests using FIPS-186-2/Rabin-Miller */
int mpRabinMiller(DIGIT_T w[], uint32_t ndigits, uint32_t t);
...
/**********************************************/
DIGIT_T mpShortAdd(DIGIT_T w[], const DIGIT_T u[], DIGIT_T d, uint32_t ndigits);
	    /* Computes w = u + d, returns carry */
DIGIT_T mpShortSub(DIGIT_T w[], const DIGIT_T u[], DIGIT_T d, uint32_t ndigits);
	    /* Computes w = u - d, returns borrow */
DIGIT_T mpShortMult(DIGIT_T p[], const DIGIT_T x[], DIGIT_T d, uint32_t ndigits);
	    /* Computes product p = x * d */
DIGIT_T mpShortDiv(DIGIT_T q[], const DIGIT_T u[], DIGIT_T d, uint32_t ndigits);
	    /* Computes q = u / d, returns remainder */
DIGIT_T mpShortMod(const DIGIT_T a[], DIGIT_T d, uint32_t ndigits);
	    /* Returns r = a mod d */
int mpShortCmp(const DIGIT_T a[], DIGIT_T d, uint32_t ndigits);
...
/************************/
uint32_t mpConvFromOctets(DIGIT_T a[], uint32_t ndigits,
				 const unsigned char *c, uint32_t nbytes);
	    /* Converts nbytes octets into big digit a of max size ndigits
	       Returns actual number of digits set */
uint32_t mpConvToOctets(const DIGIT_T a[], uint32_t ndigits,
			       unsigned char *c, uint32_t nbytes);
...
uint32_t mpConvFromDecimal(DIGIT_T a[], uint32_t ndigits, const char *s);
	    /* Convert a string in decimal format to a big digit.
	       Return actual number of (possibly zero) digits set. */
uint32_t mpConvToDecimal(const DIGIT_T a[], uint32_t ndigits, char *s,
				uint32_t smax);
...
uint32_t mpConvFromHex(DIGIT_T a[], uint32_t ndigits, const char *s);
	    /* Convert a string in hexadecimal format to a big digit.
	       Return actual number of (possibly zero) digits set. */
uint32_t mpConvToHex(const DIGIT_T a[], uint32_t ndigits, char *s,
			    uint32_t smax);
