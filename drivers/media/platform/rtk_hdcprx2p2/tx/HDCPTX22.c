...
HDCP_ERR send_ake_nostored_km(void)
{
	...
	// size: 32 bytes
	static const UINT8 sha256NullHash[SHA256_DIGEST_SIZE] = {
	...
	ps_len = 1024/8 - HDCP_KM_SIZE - 2*(SHA256_DIGEST_SIZE) - 2;
	//LOG("PS length=%d\n",ps_len);

	// generate DB = lHash || PS || 0x01 || km.
	// DB length = k - hLen - 1; (128 - 32 - 1) = 95
	db_size=SHA256_DIGEST_SIZE+ps_len+1+HDCP_KM_SIZE;
	pDB = (UINT8*)kcalloc(db_size, sizeof(unsigned char), GFP_KERNEL);
	offset=0;

	memcpy(pDB+offset,sha256NullHash,SHA256_DIGEST_SIZE);
	offset=offset+SHA256_DIGEST_SIZE;
	for(i=0;i<ps_len;i++)
	{
		pDB[offset]=0x0;
		offset++;
...
HDCP_ERR read_repeater_auth_content_stream_ready(UINT32 time_out, UINT32 *time_consumed)
{
	...
	if(IS_SUCCESS(err_code))
	{
		...
		hmacsha256(key,SHA256_DIGEST_SIZE,input,(2*hdcptx_parm.mk+HDCP_SEQ_NUM_M_SIZE),hdcptx_parm.mM);
		kfree(input);
...
HDCP_RST check_LLC_signature(void)
{
	...
	memset(out, 0x00, SHA256_DIGEST_SIZE);
