...
struct edid_info hdmitx_edid_info;
...
static bool cea_db_is_hdmi_vsdb(const u8 *db)
{
	...
	if(hdmitx_edid_info.hdmi_id!=HDMI_2P0_IDENTIFIER)//Prevent overwrite 2.0 ID
...
char * color_space_str[]={"xyYCC601","xvYCC709","sYCC601","AdobeYCC601","AdobeRGB","BT2020_cYCC","BT2020_YCC","BT2020_RGB"};
...
static void rtk_do_detailed_mode(struct detailed_timing *timing, void *c)
{
	...
	if (timing->pixel_clock) {	
		...
		{
			HDMI_DEBUG("[%s] Detailied Timing: Hdisplay(%u), Vdisplay(%u), Clock(%u), vrefresh(%u), flags(0x%x)",__FUNCTION__,
...
static void parse_hdmi_colorimetry_db(struct sink_capabilities_t *sink_cap, const u8 *db)
{
	...
	HDMI_DEBUG("[%s] color_space(0x%02x)", __FUNCTION__, sink_cap->color_space);
...
static void parse_hdmi_ycbcr420_video_db(struct sink_capabilities_t *sink_cap, const u8 *db)
{
	...
	HDMI_DEBUG("[%s] vic2_420(0x%08x%08x)", __FUNCTION__, (u32)(sink_cap->vic2_420>>32),(u32)(sink_cap->vic2_420&0xFFFFFFFF));
}
...
static bool parse_hdmi_extdb(struct sink_capabilities_t *sink_cap, const u8 *db)
{
	...
	switch (*(db+1)) {
			case VIDEO_CAPABILITY_DATA_BLOCK:
				HDMI_DEBUG("[%s] VIDEO_CAPABILITY_DATA_BLOCK (%u bytes)",__FUNCTION__,dbl);
				break;
			case VENDOR_SPECIFIC_VIDEO_DATA_BLOCK:
				HDMI_DEBUG("[%s] VENDOR_SPECIFIC_VIDEO_DATA_BLOCK (%u bytes)",__FUNCTION__,dbl);
				break;
			case VESA_DISPLAY_DEVICE_DATA_BLOCK:
				HDMI_DEBUG("[%s] VESA_DISPLAY_DEVICE_DATA_BLOCK (%u bytes)",__FUNCTION__,dbl);
				break;
			case VESA_VIDEO_TIMING_BLOCK_EXTENSION:
				HDMI_DEBUG("[%s] VESA_VIDEO_TIMING_BLOCK_EXTENSION (%u bytes)",__FUNCTION__,dbl);
				break;
			case COLORIMETRY_DATA_BLOCK:
				HDMI_DEBUG("[%s] COLORIMETRY_DATA_BLOCK (%u bytes)",__FUNCTION__,dbl);
				parse_hdmi_colorimetry_db(sink_cap,db);
				break;
			case HDR_STATIC_METADATA_DATA_BLOCK:
				HDMI_DEBUG("[%s] HDR_STATIC_METADATA_DATA_BLOCK (%u bytes)",__FUNCTION__,dbl);
				parse_hdmi_hdr_db(sink_cap, db);
				break;
			case YCBCR420_VIDEO_DATA_BLOCK:
				HDMI_DEBUG("[%s] YCBCR420_VIDEO_DATA_BLOCK (%u bytes)",__FUNCTION__,dbl);
				parse_hdmi_ycbcr420_video_db(sink_cap, db);
				break;
			case YCBCR420_CAPABILITY_MAP_DATA_BLOCK:
				HDMI_DEBUG("[%s] YCBCR420_CAPABILITY_MAP_DATA_BLOCK (%u bytes)",__FUNCTION__,dbl);
				parse_hdmi_ycbcr420_cm_db(sink_cap, db);
				break;
			case VENDOR_SPECIFIC_AUDIO_DATA_BLOCK:
				HDMI_DEBUG("[%s] VENDOR_SPECIFIC_AUDIO_DATA_BLOCK (%u bytes)",__FUNCTION__,dbl);
				break;
			case INFOFRAME_DATA_BLOCK:
				HDMI_DEBUG("[%s] INFOFRAME_DATA_BLOCK (%u bytes)",__FUNCTION__,dbl);
				break;
			default:
				HDMI_INFO("[%s] Unknow Extend Tag(%u) (%u bytes)",__FUNCTION__,*(db+1),dbl);
				break;
			}
	...
	if (cea_revision(cea) >= 3) {
		...
		for_each_cea_db(cea, i, start, end) {
			...
			case AUDIO_BLOCK:
				...
				HDMI_DEBUG("[%s] AUDIO_BLOCK (%u bytes)",__FUNCTION__,dbl);
				sad_count = dbl / 3;
				if (dbl >= 1)
					memcpy(eld + 20 + mnl, &db[1], dbl);
				break;
			case VIDEO_BLOCK:
				HDMI_DEBUG("[%s] VIDEO_BLOCK (%u bytes)",__FUNCTION__,dbl);
				break;
			case SPEAKER_BLOCK:
				/* Speaker Allocation Data Block */
				HDMI_DEBUG("[%s] SPEAKER_BLOCK (%u bytes)",__FUNCTION__,dbl);
				if (dbl >= 1){
					eld[7] = db[1];					
			...
			case VENDOR_BLOCK:
				...
				HDMI_DEBUG("[%s] VENDOR_BLOCK (%u bytes)",__FUNCTION__,dbl);
			...
			case VESA_DISPLAY_TRANSFER_BLOCK:
				HDMI_DEBUG("[%s] VESA_DISPLAY_TRANSFER_BLOCK (%u bytes)",__FUNCTION__,dbl);
				break;
			case USE_EXTENDED_TAG:
				...
				break;	
			default:
				HDMI_INFO("[%s] Unknow tag(0x%x)", __FUNCTION__, cea_db_tag(db));
				break;
			}
		}
...
int rtk_do_probe_ddc_edid(unsigned char *buf,int block,int len)
{
	...
	if(!check_hdmi_mhl_mode()){
		...
		for(i=0; i<retry;i++)
		{
			switch(i) //Slow down I2C speed when retry
			{
				case 1:
	...
	{
		struct cbus_adapter *p_adap;
		int i,msg_num,retry=5;	
		struct ddc_msg* msgs;
		int start = block * EDID_LENGTH;	
		
		msg_num = len/8;
		
		msgs = kmalloc(msg_num*sizeof(struct ddc_msg), GFP_KERNEL);
		if (!msgs)
		{	
			MHL_ERROR("malloc ddc_msg failed");
			kfree(msgs);
			//hdmitx_set_error_code(HDMI_ERROR_NO_MEMORY);
			return -1;		
		}
		
		if ((p_adap = cbus_get_adapter())==NULL){
			MHL_ERROR("get adapter failed");      
			//hdmitx_set_error_code(HDMI_ERROR_CBUS_DDC_ERROR);	
			return -ENODEV;
		}		
		...
		{
			msgs[i].segval = 0;
			msgs[i].offset = start + 8*i;
			msgs[i].devaddr = 0xA0;			
			msgs[i].buf = (buf+8*i);			
			//MHL_DEBUG("msgs[%d].offset=%d msgs[%d].devaddr=%x",i,msgs[i].offset,i,msgs[i].devaddr);
		}		
		
		for(i=0; i<retry;i++)
		{
			if (__cbus_transfer(p_adap, msgs, msg_num) == msg_num)
				return 0;
		
			msleep(500);
			MHL_ERROR("ddc retry %d time(s)",i);
		}	
...
static u8 *rtk_do_get_edid(void)
{
	...
	for (i = 0; i < 4; i++) {
		...
		{
			memcpy(&hdmitx_edid_info, block, EDID_LENGTH);// Copy Base block(128bytes) to hdmitx_edid_info
			break;
		}
	}
	if (i == 4)
